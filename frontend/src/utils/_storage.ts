...
export const saveToSessionStorage = (key: string, value: unknown, prefix = `${APP_NAME}_data`): boolean => {
  try {
...
export const getFromSessionStorage = (key: string, prefix = `${APP_NAME}_data`): unknown | undefined => {
  try {
...
export const removeFromSessionStorage = (key: string, prefix = `${APP_NAME}_data`): boolean => {
  try {
